...
class DAR_Note(SelectableListBehavior, Label):
    pass

class DAR_NoteList(SelectableList):
    def update(self, k, val):
        self.data = [{'text': str(x)} for x in self.list_data]

class DAR_NoteScreen(BaseScreen):
    '''Manages the Notes.
    ...
    def save(self, obj):
        self._save_config()
        super(DAR_NoteScreen, self).back()

    def new(self, obj):
        self._save_config()
        self.move_to('dar_new_note_screen')
