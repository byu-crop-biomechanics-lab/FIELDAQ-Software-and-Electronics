...
class ROD_Note(SelectableListBehavior, Label):
    pass

class ROD_NoteList(SelectableList):
    def update(self, k, val):
        self.data = [{'text': str(x)} for x in self.list_data]

class ROD_NoteScreen(BaseScreen):
    '''Manages the Notes.
    ...
    def save(self, obj):
        self._save_config()
        super(ROD_NoteScreen, self).back()

    def new(self, obj):
        self._save_config()
        self.move_to('rod_new_note_screen')
