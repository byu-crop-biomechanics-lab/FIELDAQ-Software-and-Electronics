...
class DAR_PointDisplay(SelectableListBehavior, Label):
    adc = NumericProperty()
    real = NumericProperty()

class DAR_PointsList(SelectableList):
    def update(self, k, val):
        self.data = [{'adc': x[0], 'real': x[1]} for x in self.list_data]

class DAR_CalibrateScreen(BaseScreen):
    ...
    def __init__(self, **kwargs):
        super(DAR_CalibrateScreen, self).__init__(**kwargs)
        self.config_data = {}
        def gui_init(dt):
            self.dar_calibrate_point_screen = self.manager.get_screen('dar_calibrate_point_screen')
    ...
    def to_points_screen(self, obj):
        self.dar_calibrate_point_screen.set_sensor(self.sensor_name)
        self.parent_screen.move_to('dar_calibrate_point_screen')
