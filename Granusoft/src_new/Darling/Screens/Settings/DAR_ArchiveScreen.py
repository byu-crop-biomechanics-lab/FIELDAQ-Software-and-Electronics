...
class DAR_TestArch(SingleSelectableListBehavior, Label):
    pass

class DAR_ArchNavButton(Button):
    pass

class DAR_TestListArch(SingleSelectableList):
    def update(self, k, val):
        self.data = [{'text': str(x)} for x in self.list_data]

class DAR_SaveTestDialogArch(Popup):
...
class DAR_SaveConfirmDialogArch(Popup):
    '''A dialog to save a file.  The save and cancel properties point to the
    functions called when the save or cancel buttons are pressed.'''
    save = ObjectProperty(None)
...
class DAR_NoUsbDialogArch(Popup):
    '''A dialog to save a file.  The save and cancel properties point to the
    functions called when the save or cancel buttons are pressed.'''
    cancel = ObjectProperty(None)

class DAR_ArchiveScreen(BaseScreen):
    ...
    def go_back(self, obj):
        super(DAR_ArchiveScreen, self).back()

    def remove_tests(self, obj):
        super(DAR_ArchiveScreen, self).move_to('dar_test_delete_confirmation')
