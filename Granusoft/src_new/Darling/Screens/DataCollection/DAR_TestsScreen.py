...
from Darling.TestSingleton import TestSingleton
...
class DAR_Test(SingleSelectableListBehavior, Label):
    pass

class DAR_NavButton(Button):
    pass

class DAR_TestList(SingleSelectableList):
    def update(self, k, val):
        self.data = [{'text': str(x)} for x in self.list_data]

class DAR_SaveTestDialog(Popup):
...
class DAR_SaveConfirmDialog(Popup):
    '''A dialog to save a file.  The save and cancel properties point to the
    functions called when the save or cancel buttons are pressed.'''
    save = ObjectProperty(None)
...
class DAR_NoUsbDialog(Popup):
    '''A dialog to save a file.  The save and cancel properties point to the
    functions called when the save or cancel buttons are pressed.'''
    cancel = ObjectProperty(None)

class DAR_TestsScreen(BaseScreen):
    ...
    def __init__(self, **kwargs):
        ...
        def gui_init(dt):
            self.test_details_screen = self.manager.get_screen('dar_test_detail_screen')
    ...
    def go_back(self, obj):
        super(DAR_TestsScreen, self).back()

    def remove_tests(self, obj):
        super(DAR_TestsScreen, self).move_to('dar_test_archive_confirmation')
    ...
    def export_tests(self, obj):
        ...
        self._popup = DAR_SaveConfirmDialog(save=self.usbSave, pathSelector=self.pathSelector, cancel=self.dismiss_popup)
        self._popup.open()
        # print("We should export all tests!")

    def pathSelector(self): # , obj):
        self.dismiss_popup()
        self._popup = DAR_SaveTestDialog(save=self.save, cancel=self.dismiss_popup)
        self._popup.open()
    ...
    def noUSB(self):
        self.dismiss_popup()
        self._popup = DAR_NoUsbDialog(cancel=self.dismiss_popup)
        self._popup.open()
    ...
    def to_test_details(self, obj):
        ...
        self.parent_screen.move_to('dar_test_detail_screen')
