...
class CAM_ImageList(SingleSelectableList):
    def update(self, k, val):
        self.data = [{'text': str(x)} for x in self.list_data]

class CAM_SaveImageDialog(Popup):
...
class CAM_ImagesViewScreen(BaseScreen):
    ...
    def remove_images(self, obj):
        super(CAM_ImagesViewScreen, self).move_to('cam_img_delete_confirmation')
    ...
    def export_images(self, obj):
        ...
        self._popup = CAM_SaveImageDialog(save=self.save, cancel=self.dismiss_popup)
        self._popup.open()
    ...
    def image_details(self, obj):
        ...
        img_screen = self.manager.get_screen('cam_img_review_screen')
        img_screen.set_image(imagename[0])
        super(CAM_ImagesViewScreen, self).move_to('cam_img_review_screen')
