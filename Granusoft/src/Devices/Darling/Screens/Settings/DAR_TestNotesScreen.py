...
class DAR_Notes(SelectableListBehavior, Label):
    pass

class DAR_NotesList(SelectableList):
    def update(self, k, val):
        self.data = [{'text': str(x)} for x in self.list_data]

class DAR_TestNotesScreen(BaseScreen):
    '''Manages the Notes.
    ...
    def save(self, obj):
        ...
        super(DAR_TestNotesScreen, self).back()

    def new(self, obj):
        self._save_config()
        self.reset_individual_test_notes()
        self.move_to('dar_new_test_note_screen')
