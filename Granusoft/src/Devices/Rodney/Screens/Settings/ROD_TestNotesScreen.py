...
class ROD_Notes(SelectableListBehavior, Label):
    pass

class ROD_NotesList(SelectableList):
    def update(self, k, val):
        self.data = [{'text': str(x)} for x in self.list_data]

class ROD_TestNotesScreen(BaseScreen):
    '''Manages the Notes.
    ...
    def on_enter(self):
        ...
        log = TestLog()
        log.connection("Entering ROD_TestNotesScreen")
    ...
    def new(self, obj):
        self._save_config()
        self.reset_individual_test_notes()
        self.move_to('rod_new_test_note_screen')
